...
import type { Certificate, Institution } from "@/lib/types";
...
export default async function InstitutionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let certificatesError: string | undefined;

  if (institution) {
    try {
      certificates = await backendFetch<Certificate[]>(
        session.institutionId,
        `/institutions/${encodeURIComponent(id)}/certificates`,
      );
    } catch (err) {
      certificatesError = err instanceof BackendError ? humanizeBackendError(err.message) : "Something went wrong loading this institution's certificates.";
    }
  }
      ...
      {error ? (
        ...
        <>
          ...
          <div className="space-y-3">
            <div>
              <h2 className="text-lg font-semibold tracking-tight">Certificates issued</h2>
              <p className="text-sm text-muted-foreground">Certificates issued by {institution.name}.</p>
            </div>
            {certificatesError ? (
              <div className="flex items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                <AlertTriangle className="size-5 shrink-0" />
              ...
              <CertificateTable certificates={certificates} showIssueAction={false} />
            )}
          </div>
        </>
      ) : null}
