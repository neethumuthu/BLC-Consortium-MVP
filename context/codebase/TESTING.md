...
- **ESLint is configured for frontend** (`eslint-config-next`'s `core-web-vitals` + `typescript` presets, `frontend/eslint.config.mjs`) — confirmed runnable. **Backend's `lint` script is not actually functional**: `package.json` has `"lint": "eslint \"{src,test}/**/*.ts\""`, but `eslint` itself is absent from `package.json`'s dependencies, absent from `node_modules`, and there is no config file anywhere in `backend/` — running `npm run lint` fails with `eslint: not found` (confirmed directly, 2026-08-12, while adding the first backend unit test; corrects this file's earlier claim that backend ESLint was "configured"). Both `tsconfig.json`s do enable `strict`-adjacent options (backend: `strictNullChecks`, `noImplicitAny`, `strictBindCallApply`; frontend: `strict: true`) — that part holds for both. There is no CI wiring found that would actually run `npm run lint` or `tsc` automatically on a PR, for either side.
