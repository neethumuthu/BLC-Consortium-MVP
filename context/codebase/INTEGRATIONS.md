...
| CouchDB | Fabric peer state database (rich queries over ledger state) | `network/generated/docker-compose-net.yaml` (one `couchdb.peer<N>.<Org>` container per peer); peers point at it via `CORE_LEDGER_STATE_COUCHDBCONFIG_*` env vars | Local only (`couchdb:3.3` image) | **Corrected 2026-08-17:** credentials are configurable (`couchdb_admin_user`/`couchdb_admin_password` in `network/deployment/local.yaml`), templated into every instance's `COUCHDB_USER`/`COUCHDB_PASSWORD`. The committed default was the well-known Fabric-tutorial value `adminpw` until rotated to a generated value ahead of making this repo public — still a static, committed value shared by every clone, not a true per-deployment secret (see `CONCERNS.md`). CouchDB ports are exposed to the host per peer (e.g. `5984`, `5994`, `6984`, `6994`, `7984`, `7994`) with no auth in front of the exposed port beyond CouchDB's own basic auth. |
