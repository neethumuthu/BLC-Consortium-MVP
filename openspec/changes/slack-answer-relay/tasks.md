...
**A fourth real bug found the same day, reported directly by Neethu rather than caught during a live attempt:** during testing, Dominik replied in several other threads in `team_blockchain` that had nothing to do with a nudge (people in this channel reflexively reply in-thread on everything). Each one correctly found no GitHub link and posted a visible "Couldn't find a linked GitHub issue in this thread" notice — technically correct, but exactly the kind of noise that makes this unusable for a team that threads normal conversation. Root cause: the relay decided whether to engage purely on whether the thread's parent *text* contained a link, never on whether the parent was actually posted by the bot in the first place. Fixed: `SlackClient.fetchThreadParent` (renamed from `fetchThreadParentText`) now also returns `isFromBot` (true only when the parent has Slack's own `bot_id` set — confirmed live that `requirements-nudge.yml`'s webhook-posted nudge has `subtype: "bot_message"` + `bot_id` set and no `user` field at all, a clean, reliable discriminator from a human-started thread). `RelayHandler.handle` now checks this before ever trying to resolve a link — a non-bot thread gets a new, deliberately silent `not_a_bot_thread` outcome, no notice posted at all, claim still released so a genuine nudge thread is unaffected.
